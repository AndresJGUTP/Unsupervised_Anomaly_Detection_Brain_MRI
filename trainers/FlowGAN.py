...
class FlowGAN(AEMODEL):
    class Config(AEMODEL.Config):
        def __init__(self):
            super().__init__('FlowGAN')
            self.scale = 10.0
            self.kappa = 1.0
            self.kl_weight = 1.0
            self.num_flows = 16
    ...
    def __init__(self, sess, config, network=None):
        ...
        self.mu = self.outputs['mu']
        self.log_var = self.outputs['log_var']

        self.z_0 = self.outputs['z_0']
        self.log_prob_z0 = self.outputs['log_prob_z0']

        # From Normalizing Flows
        self.z_k = self.outputs['z_k']
        self.log_prob_zk = self.outputs['log_prob_zk']
        self.log_det_jacobian = self.outputs['log_det_jacobian']
    ...
    def train(self, dataset):
        ...
        # loss = flows_elbo(self.z_0, self.zk, self.log_det_jacobian, self.mu, self.log_var)
        ...
        loss = tf.reduce_mean(self.log_prob_z0) + self.losses['loss_img'] - tf.reduce_mean(self.log_prob_zk) - tf.reduce_mean(self.log_det_jacobian) 
        ...
        self.losses['enc_loss'] = enc_loss = self.losses['reconstructionLoss'] + self.kl_weight * loss
